...
public class ComplexNumberCaseStudy1 {
	...
	static class Sample extends ArrayList<PopulationUnit> {
		...
		Model getModel(Double sigma2) {
			...
			double sigma2Hat = sigma2 != null ? 
					sigma2 : 
						res.transpose().multiply(res).getValueAt(0, 0) / (yVec.m_iRows - xMat.m_iCols);
			SymmetricMatrix omega = invXtX.scalarMultiply(sigma2Hat);
			return new Model(betaHat, omega, sigma2Hat);
		}
	...
	static class Model {
		...
		final SymmetricMatrix omega;
		final double sigma2Hat;
		final double sigmaHat;
		Matrix omegaChol;
		
		Model(Matrix betaHat, SymmetricMatrix omega, double sigma2Hat) {
		...
		private Matrix getOmegaChol() {
			if (omegaChol == null) {
				omegaChol = omega.getLowerCholTriangle();
			}
			return omegaChol;
		}
		...
		double getRandomDeviate(double x) {
			...
			Matrix betaDeviates = getOmegaChol().multiply(StatisticalUtility.drawRandomVector(betaHat.m_iRows, Distribution.Type.GAUSSIAN));
		...
		ComplexNumber getComplexRandomDeviate(double x) {
			...
			Matrix iPartMat = xMat.multiply(betaDeviates);
			double iPart = iPartMat.getValueAt(0, 0);
			double epsilon_b = StatisticalUtility.getRandom().nextGaussian() * sigmaHat;
			double rPart = betaHat.getValueAt(0, 0) + betaHat.getValueAt(1, 0) * x + epsilon_b;
	...
	public static void main(String[] arg) throws IOException {
		...
		String filename = ObjectUtility.getPackagePath(ComplexNumberCaseStudy1.class) + "caseStudy1.csv";
		...
		for (int real = 0; real < nbRealizations; real++) {
			...
			Model m = s.getModel(null);// null means we use the estimated variance
			double sumOLD = 0;
			double sumNEW = 0;
			for (int innerReal = 0; innerReal < nbInnerReal; innerReal++) {
				Number n1 = m.getRandomDeviate(5);
				sumOLD += n1.doubleValue() * n1.doubleValue();
				ComplexNumber n2 = m.getComplexRandomDeviate(5);
				sumNEW += n2.square().doubleValue();
			}
			sumOLD /= nbInnerReal;
			sumNEW /= nbInnerReal;
