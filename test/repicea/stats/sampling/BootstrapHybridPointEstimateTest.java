...
//	@Ignore
	...
	public void simpleTestWithCompleteVariability() {
		...
		for (int i = 0; i < NbRealizations; i++) {
			...
			for (String sampleId : pe.getPopulationUnitIds()) {
				...
				double x = pe.getObservations().get(sampleId).getValueAt(0, 0);
				obsNew.setValueAt(0, 0, x * slope + stdRes * RANDOM.nextGaussian());
				peNew.addObservation(obsNew, sampleId);
			}
		...
		System.out.println("Expected mean = " + expectedMean + " - actual mean = " + actualMean);
		Assert.assertEquals("Testing mean estimates", expectedMean, actualMean, 0.11);
		...
		System.out.println("Theoretical correction = " + theoreticalCorrection + "; Empirical correction = " + empiricalCorrection);
		Assert.assertEquals("Comparing variance bias correction", theoreticalCorrection, empiricalCorrection.getValueAt(0, 0), 2.5E-2);
	}
	...
	public void multivariateTestWithCompleteVariability() {
		...
		for (int i = 0; i < NbRealizations; i++) {
			...
			for (String obsId : pe.getPopulationUnitIds()) {
				Matrix x = pe.getObservations().get(obsId);
				...
				peNew.addObservation(obsNew, obsId);
			}
			bhpe.addPointEstimate(peNew);
		}
