	...
	class TruncatedGaussianIndividualLogLikelihood extends GaussianIndividualLogLikelihood {
		...
		class TruncationFunction extends AbstractMathematicalFunction {
			...
			public Matrix getGradient() {
				...
				double dF_ds = -1/(2*sigma2) * F_t + ErrorFunctionUtility.erf(res / Math.sqrt(2 * sigma2))/(4*sigma2) - f_t * res / (2*sigma2);
				Matrix gradient = new Matrix(TruncatedGaussianIndividualLogLikelihood.this.getNumberOfParameters(),1);
				gradient.setSubMatrix(dF_dB, 0, 0);
				gradient.setValueAt(gradient.m_iRows - 1, 0, dF_ds);
				return gradient.scalarMultiply(1 / (1-F_t));
			}
			...
			public SymmetricMatrix getHessian() {
				...
				double dF_ds = -1/(2*sigma2) * F_t + ErrorFunctionUtility.erf(erfParameter)/(4*sigma2) - f_t * res / (2*sigma2);
				double d2F_d2s = -1/(2*sigma2) * dF_ds + (1 - 2*f_t * res * (res*res - 1) - ErrorFunctionUtility.erf(erfParameter))/ (4 * sigma2 * sigma2);
				...
				d2Fd2All.setSubMatrix(d2F_dBds, d2Fd2All.m_iCols-1, 0);
				d2Fd2All.setSubMatrix(d2F_dBds.transpose(), 0, d2Fd2All.m_iRows - 1);
				d2Fd2All.setValueAt(d2Fd2All.m_iRows - 1, d2Fd2All.m_iRows - 1, d2F_d2s);
				
				Matrix hessianPart1 = gradient.multiply(gradient.transpose()).scalarMultiply(-1/(oneMinusF_t * oneMinusF_t));
